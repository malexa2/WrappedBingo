...
class Food {
	constructor(json_entry) {
		this.name = json_entry.name;
		this.image = json_entry.image;
		this.id = json_entry.id;
		this.heartsRecovered = json_entry.hearts_recovered;
		this.description = json_entry.description;
		this.effect = json_entry.cooking_effect;
		this.commonLoc = json_entry.common_locations;
	}
	...
	giveBio() {
		let s =  `${this.name}:\n${this.description}\nRecovers ${this.heartsRecovered} hearts `; 
		//console.log(this.commonLoc);
		if (this.commonLoc) {
			
			s += `and is commonly found in ${this.commonLoc}.`;
		}
		else {
			s += "and is not commonly found anywhere.";
		}
		return s;
	}
	addCommonLocation(loc) {
		if (this.commonLoc) {
			if (Array.isArray(this.commonLoc)) { //Array exists
				this.commonLoc.push(loc);
			}
			else { //Only one location
				this.commonLoc = [this.commonLoc, loc];
			}
		}
		else { //No locations
			this.commonLoc = [loc];
		}
		locMap.get(loc).addCreature(this.name);
	}
}
...
class Creature {
	...
	giveBio() {
		let s = `${this.name}\n${this.description}\n`;
		if (this.drops !== "") {
			s += "Drops " + this.drops + " when killed.";
		}
		else {
			s += "This creature does not drop anything";
		}
		return s;
	}
	addCommonLocation(loc) {
...
class Location {
	...
	getCreatures() {
		let s = `Creatures common to ${this.name} are ${this.creatures}.`;
		return s;
...
function sortCreatures() {
	//console.log("Sorting creatures");
	//Create food objects then add to foodList and location map
	data.data.food.forEach((f) => {
		foodList.push(new Food(f))
		addToLocation(f)
	});
	//sort by ID
	foodList.sort(function(a, b) {return a.id - b.id});
	//Create creature objects then add to creatureList and location map
	data.data.non_food.forEach((n) => {
		creatureList.push(new Creature(n))
		addToLocation(n)
	});
	//sort by ID
	creatureList.sort(function(a, b) {return a.id - b.id});

	//Sorts locations alphabetically
	var tmp = new Map([...locMap].sort((a, b) => String(a[0]).localeCompare(b[0])))
	locMap = tmp;
}

function addToLocation(food) {
	//console.log(food.name);
	if (food.common_locations) { 
		for (var i = 0; i < food.common_locations.length; i++) {
			//removes space from name
			var s = food.common_locations[i].replace(/\s/g, '');
			if (locMap.has(s)) { //if location exists
				locMap.get(s).addCreature(food.name);
			}
			else { //create new location if doesn't exist
				locMap.set(s,  new Location(food.common_locations[i], food.name));
			}
		}
	}
}
        ...
        <p>
            <div>
                ...
                <figure>
                    ...
                    src=
                    "https://github.com/gadhagod/Hyrule-Compendium-API/blob/master/docs/assets/light_logo.png?raw=true"
                    alt="Link"
                    >
                    </img>}
                </figure>
...
function Info(props) {
    return (
        ...
        style={{ 
            ...
            <h2>Information</h2>
            <p>Number of clicks: {props.clickCount}</p>
            <h3>Food Info</h3>
            ...
            <figure>
                ...
                <figcaption>{props.lastButton}</figcaption>
            </figure>
            </div>
...
function FoodHandler(props) {
    const [count, setCount] = React.useState(0);
    const [lastButton, setLastButton] = React.useState('Click a button!');
    const [lastImage, setLastImage] = React.useState('none');
    const handleClick = (label) => {
        setCount(count + 1);
        setLastButton(label.giveBio());
        setLastImage(label.image);
        return label;
    }
    const curFoodList = props.foodList;
    ...
    <div>
        ...
        style={{ 
			...
			{curFoodList.map((foodLabel, index) => (
                <FoodComp 
                    label = {foodLabel.name} 
                    onClick = {() => handleClick(foodLabel)} 
                    foodList = {curFoodList}
                    />
            ))}
...
//the least elegant solution I have ever coded
//I truly apologize
let data = `{
  "data": {
    "food": [
    ...
    "non_food": [
      ...
      {
        "category": "creatures",
        "common_locations": [ "Faron Grasslands", "West Necluda", "Floria Bridge" ],
      ...
      {
        "category": "creatures",
        "common_locations": [ "Hyrule Ridge", "Great Hyrule Forest", "Seres Scablands" ],
      ...
      {
        "category": "creatures",
        "common_locations": [ "Tabantha Frontier", "Akkala Highlands", "Crenel Peak" ],
      ...
      {
        "category": "creatures",
        "common_locations": [ "Hebra Mountains", "Tabantha Frontier", "Rito Stable" ],
